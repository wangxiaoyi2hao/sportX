...
-(BOOL)InsertSearchManHistory:(NSString *)searchStr{
    ...
    NSString *delectStr=[NSString stringWithFormat:@"DELETE  FROM tb_userHistory WHERE user_value='%@'",searchStr];
    char *error;
    int delectSuccess=sqlite3_exec(_datebase, [delectStr UTF8String], nil, nil, &error);
    if (delectSuccess!=SQLITE_OK) {
        NSLog(@"删除搜索历史失败");
    ...
    NSString *sqlStr=[NSString stringWithFormat:@"INSERT INTO tb_userHistory(user_value) VALUES('%@')",searchStr];
    int success=sqlite3_exec(_datebase, [sqlStr UTF8String], nil, nil, &error);
    if (success!=SQLITE_OK) {
        NSLog(@"添加搜索历史失败");
        sqlite3_close(_datebase);
...
-(BOOL)deleteSearchManHistory{
    ...
    NSString *delectStr=[NSString stringWithFormat:@"DELETE  FROM tb_userHistory"];
    char *error;
    int delectSuccess=sqlite3_exec(_datebase, [delectStr UTF8String], nil, nil, &error);
    if (delectSuccess!=SQLITE_OK) {
        NSLog(@"删除搜索历史失败");
...
-(NSMutableArray *)AllSearchManHistory{
    ...
    sqlite3_stmt *statement;
    NSString *sqlStr=[NSString stringWithFormat:@"SELECT *FROM tb_userHistory ORDER BY user_id DESC"];
